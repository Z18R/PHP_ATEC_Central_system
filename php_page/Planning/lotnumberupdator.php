...
if (isset($_POST['submitFilter'])) {
    ...
    if (!empty($data)) {
        ...
        foreach ($data as $row) {
            ...
            echo "<td>";
            // Inside the foreach loop where you display the form for updating
            echo "<form class='update-form' action='' method='post'>";
            echo "<input type='hidden' name='lotnumber' value='" . $row['lotnumber'] . "'>";
            echo "<div class='form-group'>";
            echo "<input type='text' class='form-control' id='newlotnumber' name='newlotnumber' placeholder='New Lot Number'>";
            echo "</div>";
...
if (isset($_POST['submitUpdate'])) {
    // Inside the submitUpdate condition, update the variables accordingly
    $lotNumberToUpdate = $_POST['lotnumber'];
    $newLotNumber = $_POST['newlotnumber'];

    // Here you need to construct the SQL update query to update the Commitdate with the new date
    // For example:
    //Query selecting lotnumber for update
